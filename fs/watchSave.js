...
function report(name, oldStat, newStat) {
  ...
  var changed = newStat.ctime > oldStat.ctime; // content, owner, permissions
  if (changed) {
    var modified = newStat.mtime > oldStat.mtime; // content only
    if (modified) {
      ...
      // TODO: This is never detected!  Perhaps don't even check ctimes.
      console.log(name, 'owner or permissions changed');
    }
  }
}
...
var fsw = fs.watch(dir, function (event, filePath) {
